...
| 모드 | 분량 | 용도 |
|------|------|------|
| S (퀵) | 2,000~5,000자 | 아이디어 검증 |
| M (표준) | 10,000~20,000자 | 학회 논문 수준 |
| L (풀) | 50,000자+ | 학위 논문 수준 |
...
NAVI-PJ는 **Superpowers 프레임워크** 기반의 8개 AI 워크플로우를 제공합니다.
슬래시 명령어(`/command`)로 호출합니다.

### 📑 논문 관련 워크플로우

| 명령어 | 설명 |
|--------|------|
| `/research-simulate` | **논문 시뮬레이션** — 주제만 입력하면 전체 파이프라인 자동 실행 |
| `/paper-drafting` | **논문 초안** — 기존 검색 결과로 OpenDraft 6단계 집필 |

### 🛠️ 개발 워크플로우

| 명령어 | 설명 |
|--------|------|
| `/superpowers` | **전체 개발 프로세스** — 3개의 Hard Gate (설계→테스트→커밋) 준수 |
| `/brainstorming` | **브레인스토밍** — 아이디어를 설계 문서(`docs/specs/`)로 변환 |
| `/writing-plans` | **계획 작성** — 설계를 실행 가능한 2~5분 단위 태스크로 분해 |
| `/executing-plans` | **계획 실행** — 계획을 비판적 검토 후 순차 실행 |
| `/tdd` | **TDD** — RED → GREEN → REFACTOR 사이클 |
| `/debugging` | **체계적 디버깅** — 근본 원인 4단계 분석 (추측 수정 금지) |

### 워크플로우 흐름도

```
📝 논문 프로젝트:
  /research-simulate ─────────────────────────▶ 완성된 논문 + 대시보드

🔧 개발 프로젝트:
  /brainstorming → /writing-plans → /executing-plans → 완성
       │                                    │
       └── /superpowers (전체 게이트 관리) ──┘
                  │
          /tdd (구현 시)    /debugging (이슈 시)
```
